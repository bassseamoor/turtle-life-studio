...
const ready=()=>{
 ...
 else{footer.append(node('span','su-seed-label','CREATE · EDIT · BUILD'),button('Studio ≡',()=>open(!panel.hidden)));}
 body.append(footer);
 ...
 if(isForge){move('#modeswitch',pages.World);move('#viewroot',pages.World);const aspect=$('#aspectbtn');if(aspect)pages.Camera.append(aspect);pages.Export.append(button('Export PNG',()=>{const b=document.getElementById('btnExport')||document.getElementById('eExport');if(b)b.click();}));hide('#topbar');}
 ...
 if(sc?.panel){
  const original=sc.panel;original.classList.remove('hidden');pages.World.append(original);
  const children=[...original.children];let group='World';for(const child of children){if(child.classList.contains('sc-sec')){const t=child.textContent;if(/Record video/.test(t))group='Export';else if(/Favorites|Recent/.test(t))group='World';}if(group==='Export')pages.Export.append(child);}
 ...
 const full=button('⤢ Full screen',()=>{if(document.fullscreenElement)document.exitFullscreen?.();else document.documentElement.requestFullscreen?.().catch(()=>{});});pages.Camera.append(full);
